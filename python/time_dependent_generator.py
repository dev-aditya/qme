...
fig, (ax, av) = plt.subplots(
    2, 1, figsize=(6, 3), gridspec_kw={"height_ratios": [2, 1]}
)
fig.subplots_adjust(hspace=0.1)
ax.plot(*data_hi, "g-", alpha=1, label=r"$\delta t$ = " + str(tf / len(data_hi[0])))
ax.plot(
    *data_mid, "y.--", alpha=0.7, label=r"$\delta t$ = " + str(tf / len(data_mid[0]))
)
ax.plot(
    *data_low, "r.--", alpha=0.5, label=r"$\delta t$ = " + str(tf / len(data_low[0]))
)
