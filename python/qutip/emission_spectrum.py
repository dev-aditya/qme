...
corrs_qutip = qt.correlation_2op_1t(
    qt.Qobj(H), None, times, [qt.Qobj(op) for op in c_ops], qt.Qobj(sp), qt.Qobj(sm)
)
...
bohr_freqs = np.sort(np.array([a - b for a in evals for b in evals]))
...
ax[1].set_xlim([-2, 2])
ax[1].vlines(
    bohr_freqs, 0, 18, linewidths=1, colors="black", alpha=0.2, linestyles="dashed"
)
...
ax[1].plot(wlist_qutip, spec_qutip, "k-", label="QuTiP", alpha=0.3)
ax[1].legend()
