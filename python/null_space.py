...
def Ls_tls(g_relax, g_deph):
    return [
        np.sqrt(g_relax) * np.array([[0, 1], [0, 0]]),
        np.sqrt(g_deph) * np.array([[0, 0], [0, 1]]),
    ]
...
superop = Liouvillian(
    H_tls(omega, delta), Ls_tls(g_relax, g_deph)
)  # Liouville superoperator
null = null_space(superop)
print(
    "(i) The steady-state space is a linear subpace of dimension = "
    + str(len(null.T) - 1)
)
...
null = null_space(superop)
print(
    "(ii) The steady-state space is a linear subpace of dimension = "
    + str(len(null.T) - 1)
)
