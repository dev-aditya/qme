...
def output(rho_in, th_en):
    ...
    # jump operators
    c_ops1 = [
        np.sqrt(gamma(val2 - val1)) * np.kron(np.outer(kets[k1], kets[k2]), np.eye(2))
        for k1, val1 in enumerate(vals)
        for k2, val2 in enumerate(vals)
    ]
    c_ops2 = [
        np.sqrt(gamma(val2 - val1)) * np.kron(np.eye(2), np.outer(kets[k1], kets[k2]))
        for k1, val1 in enumerate(vals)
        for k2, val2 in enumerate(vals)
    ]
    c_ops = c_ops1 + c_ops2
